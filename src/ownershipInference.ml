...
type ocon =
  ...
  | Split of ownership * (ownership * ownership)
      (** Split (o,(s,t)) is the constraint that o = s + t *)
...
type magic_loc =
  ...
  | MAlias of int  (** The ownership generated at an alias statement with id *)
[@@deriving sexp]
...
type split_loc =
  ...
  | SCall of int
      (** When splitting an argument passed in to a function at call with expression id *)
  | SRet of int
      (** When splitting a variable being returned out of a function *)
[@@deriving sexp]
...
module SplitMap : Map.S with type key = split_loc * Paths.concr_ap =
  Map.Make
    (OrderedPair
       (DefaultOrd (struct
         type t = split_loc
...
type 'a ownership_ops = {
  splits : ('a * 'a) SplitMap.t;
      (** The codomain of this map gives a pair of ownerships.
  ...
  gen : 'a GenMap.t;
      (** The codomain is the new ownership to assign to the reference identified in the second component of the key *)
}
...
type context = {
  ...
  max_vars : Std.IntSet.t;
      (** This tracks the ownership variables to
...
let unfold =
  ...
  let rec unfold_loop ~unfld = function
    | TVar id ->
        assert (IntSet.mem id unfld);
        TVar id
    | Int -> Int
    | Ref (t, o) -> Ref (unfold_loop ~unfld t, o)
    | Array (t, o) -> Array (unfold_loop ~unfld t, o)
    | Mu (id, t) when IntSet.mem id unfld -> Mu (id, unfold_loop ~unfld t)
    | Tuple tl -> Tuple (List.map (unfold_loop ~unfld) tl)
    | Mu (id, t) as mu ->
...
let%lm add_constraint ?relaxed c ctxt =
  match relaxed with
  | Some b when ctxt.relaxed = b -> { ctxt with ocons = c :: ctxt.ocons }
...
let%lm sum_ownership t1 t2 out ctxt =
  let rec loop t1 t2 out ctxt =
    ...
    | Ref (r1, o1), Ref (r2, o2), Ref (ro, oo) ->
        loop r1 r2 ro { ctxt with ocons = Split (oo, (o1, o2)) :: ctxt.ocons }
    | Tuple tl1, Tuple tl2, Tuple tl_out ->
        fold_left3i
          (fun ctxt _ e1 e2 e_out -> loop e1 e2 e_out ctxt)
          ctxt tl1 tl2 tl_out
    | Mu (_, t1'), Mu (_, t2'), Mu (_, out') -> loop t1' t2' out' ctxt
...
let rec constrain_wf_loop o t ctxt =
  match t with
  | TVar _ | Int -> (ctxt, ())
  | Tuple tl -> miter (constrain_wf_loop o) tl ctxt
...
let%lm record_alloc loc p o ctxt =
  ...
  { ctxt with op_record = { op with gen = GenMap.add (loc, p) o op.gen } }
...
let%lm record_split loc p o1 o2 ctxt =
  ...
  {
    ctxt with
    op_record = { op with splits = SplitMap.add (loc, p) (o1, o2) op.splits };
  }
...
let alloc_split, alloc_ovar =
  let alloc_ovar_inner ctxt =
    ...
    let max_vars =
      if ctxt.relaxed then ctxt.max_vars
      else
    ...
    ( {
        ...
        v_counter = ctxt.v_counter + 1;
        max_vars;
      },
      OVar ctxt.v_counter )
  in
  let alloc_split loc p o =
    let%bind o1 = alloc_ovar_inner and o2 = alloc_ovar_inner in
...
let lift_to_ownership loc root t_simp =
  let rec simple_lift ~unfld root = function
    | `Mu (id, t) when IntSet.mem id unfld ->
        let%bind t' = simple_lift ~unfld root t in
        return @@ Mu (id, t')
    | `Mu (id, t) as mu ->
        simple_lift ~unfld:(IntSet.add id unfld) root @@ unfold_simple id mu t
    | `Array `Int ->
...
let%lq lkp_ref v ctxt =
  match StringMap.find v ctxt.gamma with
  | Ref (t, o) -> (t, o)
  | _ -> failwith "Not a ref"

let%lq lkp_array v ctxt =
  match StringMap.find v ctxt.gamma with
  | Array (t, o) -> (t, o)
...
let fresh_ap e_id (p : P.concr_ap) =
  ...
  let v = match (root, suff) with Var v, `None -> v | _ -> assert false in
  ...
  loop
    (fun ?o t ->
      ...
      let%bind () =
        match o with None -> return () | Some o -> constrain_wf_loop o t'
      in
      return (t', t, t'))
    steps
...
let rec split_type loc p =
  ...
  in
  function
  | (Int as t) | (TVar _ as t) -> return (t, t)
  | Mu (id, t) ->
...
let%lm constrain_rel ~e_id ~rel ~src:t1 ~dst:t2 ctxt =
  let dst_unfld =
    ...
    if
      IntSet.mem e_id ctxt.iso.unfold_locs || IntSet.mem e_id ctxt.iso.fold_locs
    then unfold t2
    else t2
  in
  let rec loop t1 t2 ctxt =
    match (t1, t2) with
    | TVar _, TVar _ | Int, Int -> ctxt
    | Ref (t1', o1), Ref (t2', o2) | Array (t1', o1), Array (t2', o2) ->
    ...
    cont
      {
        ctxt with
        gamma =
          List.fold_left (fun acc (k, t) -> SM.add k t acc) ctxt.gamma bindings;
      }
  in
  ( {
      ctxt' with
      gamma =
        List.fold_left (fun acc (k, _) -> SM.remove k acc) ctxt'.gamma bindings;
    },
    fl )
...
let%lm sum_types t1 t2 out ctxt =
  let rec loop t1 t2 out ctxt =
    match (t1, t2, out) with
    | TVar _, TVar _, TVar _ | Int, Int, Int -> ctxt
...
let empty_ownership () = OConst 0.0
...
let rec process_expr ~output ((e_id, _), expr) =
  ...
  | Assign (v, IVar i, nxt) ->
      let%bind t2 = lkp_split (SBind e_id) i and vt, o = lkp_ref v in
      let%bind t = make_fresh_type (MGen e_id) (P.var v) (Ref (vt, o)) in
      let vt', o' =
        match t with Ref (vt', o') -> (vt', o') | _ -> assert false
      in
      [%m
  ...
  | Let (patt, rhs, body) ->
      let%bind to_bind =
        ...
        | LengthOf _ | Nondet _ | Const _ -> return Int
        (* these two cases are handled above *)
        | Null -> assert false
        | MkArray _ -> assert false
        | Read (v, _) ->
            let%bind t_cont, o = lkp_array v in
            let%bind t_cont1, t_cont2 =
              split_type (SBind e_id) (P.elem (P.var v)) t_cont
            in
            [%m
        ...
        | Deref v ->
            let%bind t, o = lkp_ref v in
            let%bind t1, t2_pre =
              split_type (SBind e_id) (P.deref (P.var v)) t
            in
            let%bind uf = is_unfold e_id in
            let t2 = if uf then unfold t2_pre else t2_pre in
            [%m
        ...
        | Mkref (RVar _) ->
            ...
            assert false
        | Mkref (RNone | RInt _) -> return @@ Ref (Int, OConst 1.0)
        | Tuple t_init ->
            let%bind tl =
              mmap
                (fun k ->
                  match k with
                  | RNone | RInt _ -> return Int
                  | RVar v ->
                      let%bind t = lkp v in
      ...
      let rec assign_patt_loop acc patt ty =
        ...
        | PVar v, _ -> (v, ty) :: acc
        | PNone, _ -> acc
        | PTuple pl, Tuple tl -> List.fold_left2 assign_patt_loop acc pl tl
...
and process_conditional ~e_id ~tr_branch ~output e1 e2 ctxt =
  ...
  let ctxt_f, ffl =
    process_expr ~output e2 { ctxt_t with gamma = ctxt.gamma }
  in
  match (tfl, ffl) with
  | `Return, f -> (ctxt_f, f)
        ...
        miter
          (fun (k, ft) ->
            ...
            [%m
              ...
              update_type k t'])
          (StringMap.bindings ctxt_f.gamma)
          { ctxt_f with gamma = StringMap.empty }
      in
      (ctxt, `Cont)
...
let analyze_fn ctxt fn =
  ...
  let start_gamma =
    SM.of_bindings @@ List.combine arg_names fn_type.arg_types
  in
  let out_type = List.combine arg_names fn_type.output_types in
  let ctxt =
    List.fold_left
      (fun ctxt ty -> fst @@ max_type ty ctxt)
      ctxt fn_type.output_types
  in
  let ctxt, _ =
    process_expr
      ~output:(Some (out_type, fn_type.result_type))
      fn.body
      { ctxt with gamma = start_gamma }
  in
  { ctxt with gamma = SM.empty }

let infer ~opts (simple_types, iso) (fn, prog) =
  let lift_plist loc l =
    mmapi (fun i t -> lift_to_ownership loc (P.arg i) t) l
  in
  let lift_simple_ft nm ft =
    ...
    and result_type =
      lift_to_ownership (MRet nm) P.ret ft.SimpleTypes.ret_type
    in
  ...
  in
  let lift_refp loc l = mmapi (fun i t -> lift_reft loc (P.arg i) t) l in
