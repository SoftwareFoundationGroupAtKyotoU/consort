...
type flow =
  | Havoc of P.concr_ap (** Havoc (i.e., make non-deterministic) the value at thegiven path *)
  | Copy of P.concr_ap * P.concr_ap (** copy the value from the first path to the second *)
  | Const of P.concr_ap * int (** Set the value at path p to the constant i *)
  | NullConst of P.concr_ap * bool (** The value at the null path p to the null flag constant b *)
...
type concr_arg =
  ...
  | KeyedChoice of P.concr_ap * concr_arg * concr_arg (** An ite expression which gives a1 if p is true, a2 other wise *)  [@@deriving sexp]

(** Formulae that appear as the components of a CHC *)
type clause =
  | PRelation of relation * ((P.concr_ap * concr_arg) list) * int option (** An application of the given relation, with a substitution described by the second argument, and an option context shift given by the third *)
  | Relation of (concr_arg,concr_arg) RT.relation * z3_types (** A logical relation (as described in refinement types) between two concrete arguments of type (z3_types) *)
  | NamedRel of string * (concr_arg list) (** A named relation with the given arguments. Generated by intrinsics; the smt function name is assumed to be built in or defined in the smt definition file for instrinsics *)
  | NullCons of concr_arg * concr_arg (** A null "well-formedness" condition, i.e., if the null flag indicated by p1 is true, then the null flag p2 must be true as well (i.e., null pointers cannot point to null pointers *)

(** 
  J.Liu: We use the boolean flags to find unfold point now, it is extremely subtle and we should do better, 
  but, whatever which approach we choose to find unfold point for types, 
  it looks like unfolding all recursive types once in both ownership and flow inference is a nice approach in the current. 
  It is very annoying in both theory and practice to do inference for recursive types without pre-unfolding in ConSORT.
*)
type fltype = [
  | `IntArray
  | `Ref of bool * fltype (** The boolean flag here indicates whether this reference was under a mu binder which has since been unfolded *)
  | `Int
  | `Tuple of bool * fltype list (** The boolean flag here indicates whether this reference was under a mu binder which has since been unfolded *)
  | `Mu of fltype
  | `TVar
]
...
let rec fltype_to_string = function
  ...
  | `Int -> "int"
  | `Tuple (_, tl) -> Printf.sprintf "(%s)" @@ String.concat ", " @@ List.map fltype_to_string tl
  | `TVar -> Greek.alpha
...
type function_info = {
  ...
  f_type: funtype;
  in_recursive_rel: recursive_ref_map; (** The map from mu binders in the arguments to mu relations *)
...
type ctxt = {
  ...
  havoc_set : P.PathSet.t; (** Havoc set is the set of havoced access paths, i.e., the set of access paths under a 0 ownership pointer. 
...
let rec unfold_fltype subst = function
  ...
  | `Tuple (false, tl) -> `Tuple (false, List.map (unfold_fltype subst) tl)
  | `Tuple (true, _) -> failwith "Already unfolded tuple under a mu binder?!?!?"
  | `IntArray -> `IntArray
...
let rec deep_type_normalization = function
  ...
  | `Mu (`Tuple (false, tl)) ->
    `Tuple (true, List.map (unfold_fltype (`Mu (`Tuple (true, tl)))) tl)
  | `Mu (`Tuple (true, _)) -> failwith "broken invariant"
  | `Mu _ -> assert false
  | `Int -> `Int
  | `Tuple (b, tl) -> `Tuple (b, List.map deep_type_normalization tl)
...
let rec simple_to_fltype ?tvar = function
  ...
  | `TVar id ->
    ...
    `TVar
  | `Tuple tl -> `Tuple (false, List.map (simple_to_fltype ?tvar) tl)
...
type copy_spec = {
  direct_copies: (P.concr_ap * P.concr_ap) list; (** copy concrete value v at path p1 to path at p2 *)
  weak_copies: (P.concr_ap * P.concr_ap * fltype) list (** Copy a recursive type of type [t] rooted at [p1] to [p2]
...
module Havoc = struct
  ...
  type havoc_state = {
    ...
    mu_havoc: mu_havoc (** The havocing under a mu binder is as described above *)
  }
  ...
  *)
  let fold_to ~oracle:(dst_oracle,src_oracle) ~e_id ~copy_policy ?rec_ext ?havoc_ext ref_ty (src_root: P.concr_ap) (target_root : P.concr_ap) in_rel out_rel ctxt =
    ...
    *)
    ...
    in
    let hstate =
      walk_type cont_ty RecRelations.MuChain.raw_stepper (handle_term dst_oracle) (P.deref target_root, None) H.empty_havoc_state
...
let%lm do_unfold_copy ~with_havoc ?(out_mapping=PPMap.id_map) ~e_id src_var dst_var (ref_ty: fltype) in_rel out_rel ctxt =
  RecursiveRefinements.unfold_to ~e_id ~with_havoc ~out_mapping ref_ty (P.var src_var) (P.var dst_var) in_rel out_rel ctxt
    ...
    *)
    let res = 
      ...
      let rec find_unfold_point to_mu ops ty =
        ...
        | `Proj _ :: remaining, `Tuple (true, _) -> (* TODO: NOW WE ASSUMEING UNFOLD MUST OCCUR BEFORE REF *)
          Some (ty, remaining, to_mu)
        | `Proj i::remaining, `Tuple (false, tl) ->
...
let infer ~opts (simple_theta,side_results) o_hints (fns,main) =
  ...
  let empty_ctxt = {
    ...
    fold_iso = IntMap.fold (fun exp_id _ set -> IntSet.add exp_id set) side_results.SimpleChecker.SideAnalysis.fold_locs IntSet.empty;
    unfold_iso = IntMap.fold (fun exp_id _ set -> IntSet.add exp_id set) side_results.SimpleChecker.SideAnalysis.unfold_locs IntSet.empty;
