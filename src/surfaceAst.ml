...
type relation = { op1 : lhs; cond : string; op2 : lhs }
type patt = A.patt
type pos = A.position
...
let tag_fresh pos t = (LabelManager.register pos, t)
let tag_with i t = (i, t)
...
let sanity_check_alias (v1 : Paths.path) (v2 : Paths.path) =
  ...
  if root1 = root2 then (
    print_string
      "Warning: found an alias statement that contains more than one \
...
let rec simplify_expr ?next ~is_tail count e : pos * A.raw_exp =
  let get_continuation ~ctxt count =
    match next with
    | None ->
        simplify_expr ~is_tail count
        @@ Value (LabelManager.register_with ctxt, `OInt 0)
    | Some e' -> simplify_expr ~is_tail count e'
  in
  match e with
  | Fail i -> A.Fail |> tag_with i
  | Unit i ->
      if is_tail then simplify_expr ~is_tail count @@ Value (i, `OInt 0)
      else A.Unit |> tag_with i
  | Value (i, v) ->
      if is_tail then
        (* lift_to_var (or lift_to_imm or lift_to_lhs)
           ...
           temporary variables.
        *)
        lift_to_var ~ctxt:i count v (fun _ tvar -> A.Return tvar |> tag_with i)
      else lift_to_var ~ctxt:i count v (fun _ _tvar -> A.Unit |> tag_with i)
  | NCond (i, v, e1, e2) ->
      A.NCond
        (v, simplify_expr ~is_tail count e1, simplify_expr ~is_tail count e2)
      |> tag_with i
  | Cond (i, `Var v, e1, e2) ->
      A.Cond
        (v, simplify_expr ~is_tail count e1, simplify_expr ~is_tail count e2)
  ...
  | Seq (pos, e1, e2) ->
      A.Seq
        (simplify_expr ~is_tail:false count e1, simplify_expr ~is_tail count e2)
      |> tag_fresh pos
  | Assign (id, v, l) ->
      lift_to_imm ~ctxt:id count l (fun c i ->
          A.Assign (v, i, get_continuation ~ctxt:id c) |> tag_with id)
  ...
  | Let (i, v, lhs, body) ->
      lift_to_lhs ~ctxt:i count lhs (fun c lhs' ->
          ...
          A.Let (v, lhs', body') |> tag_with i)
  | Alias (i, v1, v2) ->
      sanity_check_alias v1 v2;
      (* NK: added *)
      A.Alias (v1, v2, get_continuation ~ctxt:i count) |> tag_with i
  | Assert (i, { op1; cond; op2 }) ->
      lift_to_imm ~ctxt:i count
        (op1 :> lhs)
        (fun c op1' ->
          lift_to_imm ~ctxt:i c
            (op2 :> lhs)
            (fun c' op2' ->
              A.Assert
                ( { A.rop1 = op1'; A.cond; A.rop2 = op2' },
                  get_continuation ~ctxt:i c' )
              |> tag_with i))
  | Return (i, rval) ->
...
and lift_to_lhs ~ctxt count (lhs : lhs) (rest : int -> A.lhs -> A.exp) =
  ...
  | `LengthOf lhs ->
      lift_to_var ~ctxt count lhs (fun c' r -> rest c' @@ A.LengthOf r)
  | `Call c -> lift_to_call ~ctxt count c (fun c' l -> rest c' @@ A.Call l)
  | `BinOp (o1, op_name, o2) ->
      lift_to_var ~ctxt count
        (o1 :> lhs)
        (fun c i1 ->
          lift_to_var ~ctxt c
            (o2 :> lhs)
                ...
                (A.Call
                   {
                     A.callee = op_name;
                     arg_names = [ i1; i2 ];
                     label = LabelManager.register_with ctxt |> fst;
                   })))
  | `Mkref lhs ->
      lift_to_rinit ~ctxt count lhs (fun c' r -> rest c' @@ A.Mkref r)
  | `MkArray lhs ->
      lift_to_var ~ctxt count lhs (fun c i1 -> rest c @@ A.MkArray i1)
  | `Tuple tl ->
      lift_to_tuple ~ctxt count tl (fun c' tlist -> rest c' @@ A.Tuple tlist)
  | `Read (base, ind) ->
      lift_to_var ~ctxt count base (fun c b ->
          lift_to_var ~ctxt c ind (fun c' i -> rest c' @@ A.Read (b, i)))
...
and lift_to_rinit ~ctxt count (r : lhs) rest =
  ...
  | `OVar v -> k @@ A.RVar v
  | `OInt i -> k @@ A.RInt i
  | #lhs as l -> bind_in ~ctxt count l (fun c' var -> rest c' @@ A.RVar var)
...
and lift_to_tuple ~ctxt count l rest =
  let rec t_loop c acc kl =
    ...
    | [] -> rest c @@ List.rev acc
    | v :: l ->
        lift_to_rinit ~ctxt c v (fun c' lifted -> t_loop c' (lifted :: acc) l)
  in
  t_loop count [] l
...
and lift_to_var ~ctxt c (h : lhs) rest =
  match h with
  | `OVar v -> rest c v
  | _ -> bind_in ~ctxt c h (fun c' tvar -> rest c' tvar)

and lift_to_imm ~ctxt count (o : lhs) rest =
  match o with
  | `OVar s -> rest count @@ A.IVar s
  | `OInt i -> rest count @@ A.IInt i
  | _ -> bind_in ~ctxt count o (fun c tvar -> rest c (A.IVar tvar))

and lift_to_call ~ctxt count (callee, id, args) rest =
  let rec recurse c a k =
    match a with
    | [] -> k c []
    | h :: t ->
        lift_to_var ~ctxt c
          (h :> lhs)
          (fun c' var -> recurse c' t (fun c'' l -> k c'' @@ (var :: l)))
  in
  recurse count args (fun c' l ->
      rest c' { A.callee; A.label = id; A.arg_names = l })
...
and bind_in ~ctxt count lhs (k : int -> string -> A.exp) =
  lift_to_lhs ~ctxt count lhs (fun c' lhs' ->
      ...
      A.Let (A.PVar tvar, lhs', to_inst)
      |> tag_with @@ LabelManager.register_with ctxt)

let simplify (fns, body) =
  let simpl_fn =
    List.map
      (fun (name, args, e) ->
        { A.name; A.args; A.body = simplify_expr ~is_tail:true 0 e })
      fns
  in
