...
type funtyp_v = { arg_types_v : int list; ret_type_v : int }
...
let _sexp_of_resolv_map r =
  Hashtbl.fold (fun k v acc -> (k, v) :: acc) r []
  |> [%sexp_of: (int * typ) list]
...
type side_results = {
  ...
  let_types : typ Std.IntMap.t;
      (* let_types gives the type of the RHS of every let expression (by id). This is used
...
type fn_ctxt = { sub : sub_ctxt; fenv : funenv; tyenv : tyenv }
...
let init_tyenv fenv { name; args; _ } =
  ...
  List.fold_left2
    (fun acc name var -> StringMap.add name (`Var var) acc)
    StringMap.empty args arg_types_v

let add_var v t ctxt =
  if StringMap.mem v ctxt.tyenv then failwith ("variable shadowing: " ^ v)
  else { ctxt with tyenv = StringMap.add v t ctxt.tyenv }
...
let rec occurs_check sub v (t2 : typ) =
  ...
  | `TyCons _ ->
      ()
  | `Lock pte | `ThreadID pte -> SM.iter (fun _ t -> occurs_check sub v t) pte
...
let rec unify ~loc sub_ctxt t1 t2 =
  ...
  let unfold t1 =
    ...
    | `Var v ->
        Hashtbl.find_opt sub_ctxt.resolv v
        |> Option.map [%cast: typ]
        |> Option.value ~default:(`Var v)
    | _ -> t1
  in
  let raise_ill_typed_error t1 t2 =
    Locations.raise_errorf ~loc "Ill-typed; in %s could not unify %s and %s"
      sub_ctxt.fn_name (string_of_typ t1) (string_of_typ t2)
  in
  match (unfold t1, unfold t2) with
  | `Var v1, `Var v2 -> UnionFind.union sub_ctxt.uf v1 v2
  (* set partial resolution *)
  | `Var v1, (#refined_typ as t) | (#refined_typ as t), `Var v1 ->
      assign sub_ctxt v1 t
  | `Int, `Int -> ()
  (* structural recursion *)
  | (`Tuple tl1 as t1'), (`Tuple tl2 as t2') ->
      if List.length tl1 <> List.length tl2 then raise_ill_typed_error t1' t2'
      else List.iter2 (unify ~loc sub_ctxt) tl1 tl2
...
let rec resolve_with_rec sub v_set k t =
  ...
  | `TyCons t ->
      ...
      if IS.mem id v_set then k (IS.singleton id) @@ `TVar id
      else
  ...
  | `Tuple tl ->
      List.fold_left
        (fun k_acc t l is_acc ->
          resolve_with_rec sub v_set
            (fun is t' -> k_acc (t' :: l) (IS.union is_acc is))
            t)
        (fun l is -> k is @@ `Tuple l)
        tl [] IS.empty
  | `Var v when not @@ Hashtbl.mem sub.resolv v -> k IS.empty `Int
  | `Var v ->
...
let process_call ~loc lkp ctxt { callee; arg_names; _ } =
  ...
  let rec find_dup l =
    ...
    | _ :: t -> find_dup t
  in
  if find_dup sorted_args then failwith "Duplicate variable names detected";
  let { arg_types_v; ret_type_v } =
    try StringMap.find callee ctxt.fenv
    with Not_found -> failwith @@ "No function definition for " ^ callee
  in
  let len = List.compare_lengths arg_types_v arg_names in
  if len <> 0 then
    Locations.raise_errorf ~loc "mismatched argument arities for call to %s"
      callee;
  List.iter2
    (fun a_var t_var -> unify ~loc ctxt.sub (lkp a_var) @@ `Var t_var)
    arg_names arg_types_v;
  `Var ret_type_v
...
let rec process_expr ret_type ctxt ((id, loc), e) res_acc =
  ...
  let lkp n =
    ...
    with Not_found ->
      failwith
      @@ Printf.sprintf "Undefined variable %s at %s" n
      @@ Locations.string_of_location loc
  in
  let unify_var n typ = unify ~loc ctxt.sub (lkp n) typ in
  let unify_ref v t = unify ~loc ctxt.sub (lkp v) @@ fresh_cons ctxt.sub t in
  let fresh_var () =
  ...
  let record_tcons =
    ...
    let%lm impl tup_var ind pvar acc =
      {
        acc with
        t_cons = { var = tup_var; ind; unif = pvar; loc } :: acc.t_cons;
      }
    in
    impl
  in
  ...
  in
  let ( >> ) (a, f) b =
    if f then Locations.raise_errorf ~loc "Dead code" else b a
  in
  let unify_imm imm t =
    match imm with IInt _ -> unify `Int t | IVar v -> unify_var v t
  in
  match e with
  | Unit -> (res_acc, false)
  | Cond (v, e1, e2) ->
  ...
  | Let (p, lhs, expr) ->
      let res_acc', v_type =
        ...
        | Tuple tl ->
                  ...
                  | RVar v ->
                      if StringSet.mem v acc then
                        Locations.raise_errorf ~loc
                          "Duplicate variables in tuple constructor"
            ...
            @@ `Tuple
                 (List.map
                    (function RInt _ | RNone -> `Int | RVar v -> lkp v)
                    tl)
        | MkArray v ->
            unify_var v `Int;
...
let is_rec_assign sub c t' =
  ...
  let rec check_loop h_rec t =
    ...
    | `TyCons c ->
        ...
        else if IS.mem c_id h_rec then false
...
let typecheck_prog_with sub fenv (fns, body) =
    ...
    (fun { var; ind; unif; loc } ->
      ...
      | Some (`Tuple tl) ->
          if List.compare_length_with tl ind <= 0 then
            failwith
            @@ Printf.sprintf
                 "Ill-typed: expected tuple of at least length %d, got one of \
                  length %d"
                 (ind + 1) (List.length tl)
          else unify ~loc sub (`Var unif) @@ List.nth tl ind
      | Some t' ->
          Locations.raise_errorf ~loc "Ill-typed: expected tuple, got %s"
          @@ string_of_typ (t' :> typ))
    acc'.t_cons;
  ...
  ( List.fold_left
      (fun acc { name; _ } ->
        ...
        let arg_types =
          List.map get_soln @@ List.map (fun x -> `Var x) arg_types_v
        in
